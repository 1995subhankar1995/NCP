...
def FindPredictionSet(scores, tau, penalties = None, randomized = True, allow_zero_sets = False):
    print(tau, 'tau')
    I, ordered, cumsum = sort_sum(scores.numpy())
    print(cumsum.shape, I.shape, ordered.shape)
    #exit(1)
        ...
        for i in range(sizes_base.shape[0]):
            if penalties != None:
                V[i] = 1/ordered[i,sizes_base[i]-1] * \
                    (tau-(cumsum[i,sizes_base[i]-1]-ordered[i,sizes_base[i]-1])-penalties_cumsum[0,sizes_base[i]-1])
...
def coverage_size(S,targets):
    ...
    results = pd.DataFrame({'Marginal Coverage': [float(covered)/targets.shape[0]], 'Set Size': [size/targets.shape[0]]})
    return results
...
class SplitCP():
    ...
    def prediction1(self, SoftmaxScores = None, scores_simple=None, thresholds=None, Vanilla = True):
        n = scores_simple.shape[1]
        predicted_sets = []
        if Vanilla:
            for p in range(len(self.ScoresList)):
                if self.ScoresList[p] == 'HPS':
                    S_hat_simple = [np.where(scores_simple[p, i, :] <= thresholds['HPS'])[0].tolist() for i in range(n)]
                    #print(f"see 0 = {S_hat_simple[0]}")
                ...
                elif self.ScoresList[p] == 'APS':
                    S_hat_simple = FindPredictionSet(SoftmaxScores, thresholds['APS'], penalties = None, randomized = True, allow_zero_sets = False)
    ...
    def weighted_quantile_faster(self, arr, weights, q):
        #indices = np.argsort(cal_score, axis = 1)
        #print('s1', arr.shape, weights.shape)
        #exit(1)
        idx = np.argsort(arr)[0]
        #print(f"len = {len(idx)}")
        arr, weights = arr[:, idx], weights[:, idx]
        return self.weighted_quantile_faster2(arr, weights, q)

    def weighted_quantile_faster2(self, arr, weights, q):
        qs = np.cumsum(weights, axis = 1)
        idx = [bisect.bisect_left(qs[i], q, lo=0, hi=len(qs[i])-1) for i in range(qs.shape[0])]
        #print(arr.shape, len(idx))
        return arr[0][idx]

    def ProbabilisticLCP(self, DataLoader, num_of_classes):
        fx_test, y_test = get_logits_targets(self.model, DataLoader, BatchSize = self.BatchSize, NumClass = num_of_classes, device = self.device)
        ...
        print(f"T probabilistic value = {self.T}")
        #exit(1)
        n_test = len(y_test)
        #print(f"len = {n_test}")
        indices = torch.arange(n_test)
        # get base scores of whole clean test set
        scores_simple_clean_test = self.get_scores(fx_test, y_test, indices, num_of_classes, self.T, Vanilla = True)
        print(f"scores_simple_clean_test shape = {scores_simple_clean_test.shape} LCP")
        ...
        for experiment in tqdm(range(self.NumExperiments)):
            ...
            #distance_mat = torch.exp(
            #-(torch.cdist(x_test[idx2].reshape(x_test[idx2].shape[0], -1), x_test[idx1].reshape(x_test[idx1].shape[0], -1), p=2)) / 1.0)

            distance_mat = 1 -  torch.exp(-(torch.cdist(fx_test[idx2], fx_test[idx1], p=2)) / 1.0)

            distance_mat = distance_mat/torch.sum(distance_mat, keepdim = True, dim = 1)
            
            #print(torch.sum(distance_mat, dim = 1))
            #exit(1)
            distance_mat = distance_mat.numpy()
            cal_score = scores_simple_clean_test[:, idx1, y_test[idx1]]
            if len(self.ScoresList) > 1:
                thresholds = np.zeros((len(self.ScoresList), len(idx2)))

                for i in range(len(self.ScoresList)):
                    thresholds[i, :] = self.weighted_quantile_faster(np.expand_dims(cal_score[i], axis = 0), distance_mat, 1 - self.alpha)

            else:
                thresholds = self.weighted_quantile_faster(cal_score, distance_mat, 1 - self.alpha)
                thresholds = np.expand_dims(np.array(thresholds), axis = 0)
                
            #print(f"thresholds shape = {thresholds.shape}")
            #print(f"Probabilistic LCP threshold = {np.mean(thresholds, axis = 1)}")
            #exit(1)

            predicted_clean_sets = self.prediction_ProbabilisticLCP(scores_simple=scores_simple_clean_test[:, idx2, :], thresholds=thresholds)
    ...
    def RegularizeProbabilisticLCP(self, DataLoader, num_of_classes, k_reg = None, lamda = None):
        fx_test, y_test = get_logits_targets(self.model, DataLoader, BatchSize = self.BatchSize, NumClass = num_of_classes, device = self.device)
        ...
        for experiment in tqdm(range(self.NumExperiments)):
            ...
            if k_reg == None or lamda == None:
                ...
                for temp_lam in [0.001, 0.01, 0.1, 0.2, 0.5]:
                    scores_simple_clean_test = self.get_scores(fx_test, y_test, indices, num_of_classes, self.T, temp_lam, k_reg, Vanilla = False)
                    #print(f"scores_simple_clean_test shape = {scores_simple_clean_test.shape} RLCP")

                    distance_mat = 1 -  torch.exp(-(torch.cdist(fx_test[idx_hyper], fx_test[idx_cal], p=2)) / 1.0)
                    ...
                    cal_score = scores_simple_clean_test[:, idx_cal, y_test[idx_cal]]
                    #print(f"cal_score shape = {cal_score.shape}")
                    #exit(1)

                    thresholds = self.weighted_quantile_faster(cal_score, distance_mat, 1 - self.alpha)
                    thresholds = np.expand_dims(np.array(thresholds), axis = 0)
                    #print(f"thresholds shape = {thresholds.shape}")
                    #print(f"Regularized Probabilistic LCP threshold = {np.mean(thresholds, axis = 1)}")
                    #exit(1)
            ...
            print(f"lamda_star = {lamda_star}")
            scores_simple_clean_test = self.get_scores(fx_test, y_test, indices, num_of_classes, self.T, lamda_star, k_reg, Vanilla = False)

            distance_mat = 1 -  torch.exp(-(torch.cdist(fx_test[idx_test], fx_test[idx_cal], p=2)) / 1.0)
    ...
    def HypertuningBothLamdas(self, fx_test, idx_hyper, idx_cal, scores_simple_clean_test, y_test, ):
        ...
        KNNs = np.linspace(500, len(idx_cal), 5)
